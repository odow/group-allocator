...
histogram( ~ Cumulative.GPA | factor(Allocated.Group, labels=labels_2013), data=manv.df, breaks=gpa_bins, ylab=ylabp,GPA=xlabp, main="Manual Allocation GPA Histogram for Groups in 2013")

# 2013 Group-Allocator
autov.df <- read.csv("2013_group_allocator_r.csv",header=TRUE)
histogram( ~ Cumulative.GPA | factor(Allocated.Group, labels=labels_2013), data=autov.df, breaks=gpa_bins, ylab=ylabp,GPA=xlabp, main="Automatic Allocation GPA Histogram for Groups using 2013 Data")
